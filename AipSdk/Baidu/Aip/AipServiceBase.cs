...
namespace Baidu.Aip
{
    ...
    public abstract class AipServiceBase
    {
        ...
        protected virtual JObject PostAction2(AipHttpRequest2 aipReq)
        {
            var respStr = SendRequet2(aipReq);
            JObject respObj;
            try
            {
                respObj = JsonConvert.DeserializeObject(respStr) as JObject;
        ...
        protected string SendRequet2(AipHttpRequest2 aipRequest)
        {
            return aipRequest.ContentEncoding.GetString(aipRequest.GenerateDevWebRequest(Token));
        }
